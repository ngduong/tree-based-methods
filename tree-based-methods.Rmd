...
From the confusion matrix, we can calculate the test error rate as: (30+37)/(187+37+30+102) = 0.188.
...
rpart.fit <-train(purchase~., oj_train,
                  method = "rpart",
...
The above classification is tuned by using caret.
...
We can observe similar results as in RF model regarding variable importance. "loyal_ch" (customer brand loyaltyf for Citrus Hill), price_diff (sale price of MM minus sale price of CH) are still the two most influential features, whereas percent discount and discount offered for Citrus Hill are still among the least important variables. 
...
**Plots to compare train model by caret**
```{r}
